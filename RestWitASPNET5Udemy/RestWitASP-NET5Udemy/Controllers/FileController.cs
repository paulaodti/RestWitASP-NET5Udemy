...
namespace RestWitASP_NET5Udemy.Controllers
{
    ...
    public class FileController : Controller
    {
        ...
        [HttpPost("uploadFile")]
        [ProducesResponseType(typeof(FileDetailVO),200)]
        ...
        [ProducesResponseType(typeof(List<FileDetailVO>),200)]
        [ProducesResponseType(400)]
        [ProducesResponseType(401)]
        [Produces("application/json")]
        public async Task<IActionResult> UploadManyFiles([FromForm] List<IFormFile> files)
        {
            List<FileDetailVO> fileDetail = await _fileBusiness.SaveFilesToDisk(files);
            return new OkObjectResult(fileDetail);
        }
